...
comprDeptEmployees [] dept = []

comprDeptEmployees (x:xs) dept = 
  [ x | x <- employees , (emplDept x)== dept]
...
expn x = expnAux x 0
expnAux x n = ((x**n)/(product[1..n])) : expnAux x (n+1)
...
charIndexes str char = [ i | (i,c) <- zip [0..] str, c==char]
...
foldTree treefn leafFn (Tree leftTree node rightTree) =  treefn (foldTree treefn leafFn leftTree)  node (foldTree treefn leafFn rightTree)
foldTree treefn leafFn (Leaf node) = leafFn node
...
flattenTree = foldTree(\leftTree node rightTree -> leftTree ++ node : rightTree) (\node -> [node])
...
catenateTreeLists btree = concat(flattenTree btree)
