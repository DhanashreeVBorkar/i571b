...
dept_employees1(Dept, [H|T]) -> 
   if Dept == H#employee.dept -> [H|dept_employees1(Dept,T)];
     true -> dept_employees1(Dept,T)
  end.
...
lists:filter(fun (E) -> E#employee.dept == Dept end, Employees).
...
% [E] appended.
%upsert_employee(E, Employees) -> .
upsert_employee(E, []) -> [E];
upsert_employee(E1, [H|T]) -> 
  if E1#employee.name == H#employee.name -> [E1|T];
     true -> [H|upsert_employee(E1,T)]
 end.



% upsert_employee(E, Employees) ->
%     % create a list of employees with the same name as the input employee
%     % E. if this list is empty, it means that the employee does not exist
%     % in the Employees list
%     Employees1 = lists:filter(fun(X) -> X#employee.name == E#employee.name end, Employees),
%     case length(Employees1) of
%         0 -> % employee does not exist, append it to the Employees list
%             Employees ++ [E];
%         _ -> % employee exists, replace the existing employee with the input
%             % employee E
%             [E | [ X || X <- Employees, X#employee.name /= E#employee.name ]]
%     end.
%% returns list of pairs: { Args, Result }, where Args is list of
%% arguments to function and Result should be the value returned
%% by the function.
  ...
  case Req of
    ...
    { read, Name } ->
        case find_employees([employee_has_name(Name)], Employees) of
            ...
            _ -> { err, "No employee found with the given name", Employees }
        end;
    { upsert, Employee } ->
...
employees_req_with_sort(Req, Employees) ->
    case Req of
        {sort} ->
            % sorting the employees by name in ascending order
            SortedEmployees = lists:sort(fun(E1, E2) ->
                employee_name(E1) < employee_name(E2)
            end, Employees),
            {ok, void, SortedEmployees};
        _ -> employees_req(Req, Employees)
    end.

employee_name({employee, Name, _Age, _Department, _Salary}) ->
    Name.
...
start_employees_server(Employees, Fn) ->
  SPid = spawn(fun () -> prj5_sol:server_loop(Fn, Employees)end),
  register(emps, SPid),
  SPid.

server_loop(Fn, Employees) ->
  receive
    {SPid, Req} ->
      case Req of
        {stop} -> SPid ! {self(), {ok, void}},
          ok;
        {new_fn, Func1} -> SPid ! {self(), {ok, void}},
          server_loop(Func1, Employees);
        _ ->{Status, Result, Employees1} = Fn(Req, Employees),
          SPid ! {self(), {Status, Result}},
          server_loop(Fn, Employees1)
      end
  end.
...
%stop_employees_server() -> 'TODO'.
  ...
  stop_employees_server() -> emps ! {stop},
  unregister(emps),
  {ok, stopped}.
...
employees_client(Req) -> Pid = whereis(emps),
Pid ! { self(), Req },
receive
{ Pid,{Status, Result} } -> {Status, Result}
end.
